    <div wire:id="" class="row">
                        ...
                        <input type="search"
                        class="form-control font-weight-bolder tw-rounded-lg-r" wire:model.debounce.500ms="search"
                        placeholder="Search Owner Or Discount Title ">
                        ...
                        <!-- Start Left Menu-->
                                        ...
                                        {{-- <div class="row" style="padding: 5px;">
                                            <div class="col-12" >
                                                <div class="form-check form-check-inline">
                                                    ...
                                                    <label class="form-check-label" for="inlineRadio1">Search By Owner</label>
                                                  </div>
                                                  <div class="form-check form-check-inline">
                                                    <input class="form-check-input" type="radio" name="inlineRadioOptions" id="inlineRadio2" value="option2">
                                                    <label class="form-check-label" for="inlineRadio2">Search BY Title</label>
                                                  </div>
                                ...
                                @foreach ($discountCategories as $item)
                                    <div class="bg-light-light shadow-none rounded-0 my-2 p-3 d-flex justify-content-between align-items-start">
                                <h5 class="mb-0">
                                    <span class="svg-icon">
                                ...
                                <label class="checkbox checkbox-info" wire:key="discount_{{ $item->id }}">
                                    <input type="checkbox" value="{{ $item->id }}" id="{{ $item->id }}" wire:model="category_name" name="category_name[]"/>

                                    <span class="border-info border-2 rounded-sm"></span>
                                </label>

                                {{-- <label for="{{ $item->category_name }}" class="checkbox checkbox-info" wire:key="{{ $item->id }}">
                                    <input type="checkbox" id="{{ $item->category_name }}" value="{{ $item->category_name }}" wire:model="category_name"  name="{{ $item->category_name }}">
                                    <span class="border-info border-2 rounded-sm"></span>
...
<div class="d-flex mt-5 flex-column flex-md-row align-items-center mb-9 card card-body border-light overflow-hidden  dp-card" style="padding: 2em">
    <div class="" style="border-radius: 11.05px; border:2px solid #2A337E; font-size: 2em; padding:1em 1.7em">
        <b style="font-size: 1.5em">{{ $discount->discount }}%</b>  <br> <b>OFF</b>
        <br>
        <span style="font-size: 10px" class="badge badge-info">Coupons</span>
    </div>
{{-- <img class="w-md-125px w-100 h-125px object-cover align-self-start mr-md-2 rounded-lg shadow-sm" src="{{
    $discount->cover_photo ? asset('storage/discount_img/'.$discount->cover_photo) : 'media/job-opportunities.png'  }}

    " alt=""> --}}
<!--begin::Title-->
<div class="d-flex flex-column flex-grow-1 mx-2" style="padding-left:2em">
                                <span class="font-weight-boldest text-dark font-size-lg mb-1 font-size-h4">
                                    {{ $discount->discount }}  %  {{ $discount->title }}

                                </span>
            <div>
                @if(is_array($discount->services))
...
<a href="#" wire:click="showDiscountDetail({{ $discount->id }})" class="btn btn-info rounded border-2 my-2 w-md-180px w-100">
    Get coupon code
</a>
        ...
        <div class="col-md-6">
            ...
            <form>
                <div class="form-group">
                  <label >Title</label> <small style="float:right;" class="form-text">Require.</small>
                  <input type="text" wire:model="title" class="form-control" style="border:solid 1px #121B65;" required>
                  @error('title')
                ...
                <div class="form-group">
                    <label >Description</label> <small style="float:right;" class="form-text">Min.200 Characters.</small>
                    <textarea type="text"  wire:model="description"  rows="5"  class="form-control" style="border:solid 1px #121B65;" required>
                    </textarea>
                         @error('description')
                  <span class="text-danger">{{ $message }}</span>
                  @enderror
                ...
                <div class="form-group">
                <label >Cover Picture</label> <small style="float:right;" class="form-text">Require.</small>
                  <div class="custom-file">
                    <input type="file"  wire:model.defer="cover_photo" name="cover_photo" class="custom-file-input" >
                    <label class="custom-file-label" for="customFile" style="border:solid 1px #121B65;" required> </label>
                ...
                <div class="form-group">
                    ...
                    <input type="checkbox" wire:model="category" value="{{ $discountCategory->id }}" name="category[]" >{{ $discountCategory->category_name }}
...
<br>
<button class="btn btn-info tw-rounded-l-lg" wire:click="saveAsDraft" type="button">Save As Draft</button>
<button class="btn btn-primary m-5" wire:click="publishDiscount" type="button">Publish Discount</button>
        </form>
...
<div class="container">
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input type="number" wire:model="discount" class="form-control" style="border:solid 1px #121B65;" required>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        <label >Coupons</label>
                        <input type="text" wire:model.lazy="coupon" class="form-control" style="border:solid 1px #121B65;" required>
...
<div class="row">
                <div class="col-md-4">
                    <div class="form-group">
                        <label >From</label>
                        <input type="date" wire:model="from" class="form-control" style="border:solid 1px #121B65;" required>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        <label >To</label>
                        <input type="date" wire:model="to" class="form-control" style="border:solid 1px #121B65;" required>
...
<div class="form-group">
 ...
 &nbsp;
<label class="checkbox checkbox-info">
<input type="checkbox" wire:model="services" name="services[]" value="{{ $service->id }}">{{ $service->name }}
<span class="rounded-sm"></span></label>
@endforeach
...
<h2 style="margin: 2em">{{ $discount->title }}</h2>
<p style="margin: 2em">{{ $discount->description }}</p>
    <div class="d-flex mt-5 flex-column flex-md-row align-items-center ">
<div class="" id="CouponCode" style="border-radius: 11.05px; border:2px solid #2A337E; font-size: 2em; padding:1em; display: flex;margin: 2em">
        {{ $discount->coupons }}

        <div class="" style="margin-left: 3em">
            <button class="btn btn-info" onclick="copyCode('CouponCode')">Copy and Apply</button>
        </div>
    </div>
