<div class="min-h-450px">
    ...
    <ul class="nav nav-pills custom-navs justify-content-between" id="myTab" role="tablist">
        ...
        <li class="nav-item">
            <a class="nav-link rounded-0 text-info text-left pl-0 {{ getActiveClass($tab, 'dp') }}" id="dp-tab"
                data-toggle="tab" wire:click="$set('tab','dp')" href="#dp" role="tab" aria-controls="messages"
                aria-selected="false">
                {{ __('Web/Mobile apps') }}
            </a>
        </li>
        ...
        <li class="nav-item">
            <a class="nav-link rounded-0 text-info text-left pl-0 {{ getActiveClass($tab, 'dsp') }}" id="home-tab"
                data-toggle="tab" wire:click="$set('tab','dsp')" href="#all" role="tab" aria-controls="home"
                aria-selected="true">{{ __('Tech Business') }}
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link rounded-0 text-info text-left pl-0 {{ getActiveClass($tab, 'msme') }}" id="profile-tab"
                data-toggle="tab" wire:click="$set('tab','msme')" href="#msmes" role="tab" aria-controls="profile"
                aria-selected="false">{{ __('Busines Directory') }}</a>
        </li>
        <li class="nav-item">
            <a class="nav-link rounded-0 text-info text-left pl-0 {{ getActiveClass($tab, 'iworker') }}"
                id="settings-tab" data-toggle="tab" wire:click="$set('tab','iworker')" href="#iworkers" role="tab"
                aria-controls="settings" aria-selected="false">{{ __('app.iWorkers') }}</a>
        </li>

        <li class="nav-item ">
            <a class="nav-link rounded-0 text-info text-left pl-0 {{ getActiveClass($tab, 'comparison') }}"
                data-toggle="tab" wire:click="$set('tab','comparison')" href="#comparison_tab">
                {{ __('Digital Finance') }}
            </a>
        </li>
        <li class="nav-item ">
            <a class="nav-link rounded-0 text-info text-left pl-0 {{ getActiveClass($tab, 'events') }}"
                data-toggle="tab" wire:click="$set('tab','events')" href="#events_tab">
                {{ __('app.Events') }}
            </a>
        </li>
        <li class="nav-item ">
            <a class="nav-link rounded-0 text-info text-left pl-0 {{ getActiveClass($tab, 'discounts') }}"
                data-toggle="tab" wire:click="$set('tab','discounts')" href="#discounts_tab">
                {{ __('Discounts') }}
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link rounded-0 text-info text-left pl-0 {{ getActiveClass($tab, 'opportunities') }}"
                id="opportunities-tab" data-toggle="tab" wire:click="$set('tab','opportunities')" href="#opportunities"
                role="tab" aria-controls="settings" aria-selected="false">
                {{ __('app.Opportunities') }}
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link rounded-0 text-info text-left pl-0 {{ getActiveClass($tab, 'news') }}" id="news-tab"
                data-toggle="tab" wire:click="$set('tab','news')" href="#news" role="tab" aria-controls="settings"
                aria-selected="false">
                {{ __('News') }}
            </a>
        </li>
    ...
    <div class="tab-content py-4">
        ...
        <div class="tab-pane {{ getActiveClass($tab, 'news') }}" id="news_tab" role="tabpanel"
            ...
            @if ($tab == 'news')
                <livewire:v2.home-tabs.news-details />
            @endif
        </div>
    </div>
