...
class MsmeTab extends Component
{
    ...
    public function render()
    {
        ...
        $clients = Client::with(['application.businessSectors', 'ratings', 'application.msmeRegistration'])
            ...
            ->whereHas('application', function (Builder $builder) use ($search) {
                ...
                $builder->when(!empty($search), function (Builder $builder) use ($search) {
                    $builder->whereHas('msmeRegistration', function (Builder $builder) use ($search) {
                ...
                $builder->when($this->provinceId > 0, function (Builder $query) {
                    $query->whereHas('msmeRegistrations', function (Builder $query) {
        ...
        $businessSectors = BusinessSector::query()
            ->get();
        return view('livewire.v2.home-tabs.msme-tab', [
            'clients' => $clients,
