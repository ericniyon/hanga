...
//        $this->guard()->login($user);
    ...
    public function otpVerify(Request $request): RedirectResponse
    {
        $request->validate([
            ...
            'otp.exists' => 'Invalid code provided'
        ]);
        ...
        if (is_null($client))
            return back()->withInput()->with(['error' => "Invalid code provided "]);

        if ($client->expires_at < now())
            return back()->withInput()->with(['error' => "The code provided has been expired, you can resend the code to get a new one."]);
        ...
        $route = route('client.change.password', ['token' => $token]);
    ...
    {
        return view('client.auth.passwords.change', compact('token'));
    }
    ...
    public function savePassword(ValidatePasswordRegistration $request): RedirectResponse
    {
        ...
        if (is_null($client)) {
            return back()->withErrors($request, ['password' => 'Invalid token provided']);
        }
